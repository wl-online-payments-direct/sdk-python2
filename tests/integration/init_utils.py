...
API_KEY_ID = os.getenv("direct.api.apiKeyId")
SECRET_API_KEY = os.getenv("direct.api.secretApiKey")
MERCHANT_ID = str(os.getenv("direct.api.merchantId"))
if API_KEY_ID is None:
    raise EnvironmentError("could not access environment variable direct.api.apiKeyId required for testing")
if SECRET_API_KEY is None:
    raise EnvironmentError("could not access environment variable direct.api.secretApiKey required for testing")
if MERCHANT_ID == 'None':
    raise EnvironmentError("could not access environment variable direct.api.merchantId required for testing")
